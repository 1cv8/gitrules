...
Процедура РекурсивноРазобратьДеревоПравил(Элемент, ИмяЭлемента, ТекущийКаталог, МассивРеквизитовУзла = Неопределено, Знач Префикс = "") // РСВ 28/08/23 + Префикс
	...
	Для Каждого ТекущаяСтрока Из Элемент.Строки Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Префикс) Тогда
				Идентификатор = "" + Префикс + "_" + Идентификатор;
			КонецЕсли;
			...
			Если (ИмяЭлементаСтроки = "Параметры" И НЕ ТекущаяСтрока.Родитель.Имя = "Алгоритм")
				...
				РекурсивноРазобратьДеревоПравил(ТекущаяСтрока, ИмяЭлементаСтроки, ИмяКаталогаСтроки,, Префикс);
				
			ИначеЕсли ИменаСобытийКонвертации.Найти(ИмяЭлементаСтроки) <> Неопределено Тогда
				
				ИмяКаталогаСобытий = СоздатьКаталогСобытий(ТекущийКаталог, Префикс);		
			...
			ИначеЕсли ИмяЭлементаСтроки = "Группа" Или ИмяЭлементаСтроки = "Свойства" Или ИмяЭлементаСтроки = "Значения" Тогда
				...
				РекурсивноРазобратьДеревоПравил(ТекущаяСтрока, ИмяЭлементаСтроки, ИмяКаталогаСтроки, МассивРеквизитовТекущегоУзла, Префикс);
			...
			ИначеЕсли ИмяЭлементаСтроки = "Свойство" 
				...
				НовПрефикс = "";
				Если ЗначениеЗаполнено(Префикс) Тогда
					НовПрефикс = Префикс + "_";
				КонецЕсли;
				НовПрефикс = НовПрефикс + ИмяЭлементаСтроки;
				НовПрефикс = "" + НовПрефикс + "_" + СокрЛП(Идентификатор);
				
				РекурсивноРазобратьДеревоПравил(ТекущаяСтрока, ИмяЭлементаСтроки, ИмяКаталогаСтроки, МассивРеквизитовТекущегоУзла, НовПрефикс);
...
Функция СоздатьКаталогСобытий(ТекущийКаталог, Префикс = "") // РСВ 28/08/23 + Префикс
	
	// РСВ 28/08/23 + Префикс
	Если ЗначениеЗаполнено(Префикс) Тогда
		ИмяКаталога = ОбъединитьПути(ТекущийКаталог, Префикс + "_" + НаименованиеКаталогаСобытий);
	Иначе
